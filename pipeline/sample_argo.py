...
INDEX = f"{GDAC}/ar_index_this_week_prof.txt"


def choose_profiles(text: str, since: str, limit: int) -> list[dict]:
    ...
    for raw in reversed(text.splitlines()):
        ...
        if date[:8] < since_compact or not (45 <= lat <= 65 and -60 <= lon <= -10) or float_id in seen_floats:
            continue
...
def main() -> None:
    ...
    manifest = choose_profiles(index_text, args.since, args.limit)
    completed = []
    for record in manifest:
        ...
        completed.append({**record, "local": str(target), "bytes": target.stat().st_size})
    (destination / "manifest.json").write_text(json.dumps({"index": INDEX, "bounds": [45, -60, 65, -10], "since": args.since, "profiles": completed}, indent=2) + "\n")
